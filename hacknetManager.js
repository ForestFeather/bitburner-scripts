...
export async function main(ns) {
	...
	let maxNodes = ns.args.length > 0 ? ns.args[0] : 8;
	let maxLevel = ns.args.length > 1 ? ns.args[1] : 200;
	let maxRam = ns.args.length > 2 ? ns.args[2] : 64;
	let maxCores = ns.args.length > 3 ? ns.args[3] : 16;
	var nodes = 0;
	var ref = 0;
	let nodesMaxed = 0;
	ns.disableLog("ALL");
