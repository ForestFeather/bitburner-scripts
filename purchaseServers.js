...
export async function main(ns) {
  ...
  while (true) {
    ...
    if (action == settings.actions.BUY) {
      ...
      targetRam = Math.min(targetRam, ns.getPurchasedServerMaxRam())
      ...
      if (smallestCurrentServer === ns.getPurchasedServerMaxRam()) {
          ...
          if (targetRam > ns.getServerRam(purchasedServers[0]).shift()) {
            if (ns.getServerMoneyAvailable('home') * settings.totalMoneyAllocation >= ns.getPurchasedServerCost(targetRam)) {
              ...
              if (serverDeleted) {
                ...
                if (hostname) {
				  let serverCost = ns.getPurchasedServerCost(targetRam);
				  let serverCostMoney = serverCost.toLocaleString('en-US', { style: 'currency', currency: 'USD' });
                  ns.tprint(`[${localeHHMMSS()}] Upgraded: ${purchasedServers[0]} into server: ${hostname} (${targetRam} GB) for ${serverCostMoney}`)
