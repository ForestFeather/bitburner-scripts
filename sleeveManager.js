...
const argsSchema = [
	...
	['reserve', 0], // Reserve this much cash before determining spending budgets (defaults to contents of reserve.txt if not specified)
...
async function mainLoop(ns) {
	...
	let budget = (playerInfo.money - (argsSchema['reserve'] || globalReserve)) * argsSchema['aug-budget'];
...
async function manageSleeveAugs(ns, i, budget) {
	...
	if (availableAugs[i] == null || Date.now() > cacheExpiry[i]) {
		...
		availableAugs[i] = (ns.sleeve.getSleevePurchasableAugs(ns.args[i])  // list of { name, cost }
	...
	if (cooldownLeft == 0 && batchCount > 0 && ((batchCount >= availableAugs[i].length - 1) || batchCount >= argsSchema['min-aug-batch'])) { // Don't require the last aug it's so much more expensive
		...
		if (ns.args.slice(1).reduce((s, aug) => s && ns.sleeve.purchaseSleeveAug(ns.args[i], aug), true)) {
...
async function pickSleeveTask(ns, playerInfo, i, sleeve, canTrain) {
	...
	var crime = argsSchema.crime || (await calculateCrimeChance(ns, sleeve, "Homicide")) >= argsSchema['homicide-chance-threshold'] ? 'Homicide' : 'Mug';
